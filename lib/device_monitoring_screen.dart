...
class _DeviceMonitoringScreenState extends State<DeviceMonitoringScreen> {
  ...
  Future<void> _saveDevices() async {
    ...
    final String devicesJson = jsonEncode(
      _allDevices.map((device) => device.toJson()).toList(),
    );
  ...
  Future<void> _fetchDevices() async {
    ...
    try {
      ...
      try {
        ...
        for (var device in _allDevices) {
          device.status = currentOnlineIps.contains(device.ip)
              ? DeviceStatus.online
              : DeviceStatus.offline;
        }
  ...
  void _showDisconnectedDevices() {
    setState(() {
      ...
      if (_displayedDevices.isEmpty)
        showErrorSnackBar(context, 'لا توجد أجهزة غير متصلة حالياً.');
    });
  }
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  color: device.status == DeviceStatus.online
                      ? Colors.green.shade800.withAlpha((255 * 0.8).round())
                      : Colors.grey.shade700.withAlpha((255 * 0.8).round()),
                  child: ListTile(
                    leading: Icon(
                      device.status == DeviceStatus.online
                          ? Icons.circle
  ...
  Future<void> _checkSingleDevice(Device device) async {
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text('جاري فحص ${device.name}...')));
