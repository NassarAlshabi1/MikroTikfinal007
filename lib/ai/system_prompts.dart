...
- **بوابة RouterOS v6**: أي أمر يجب أن يمر عبر Native Binary API على المنفذ 8728.
- **أمر تحقق قراءة فقط**: استخدم `print`/`monitor`/`export` للقراءة فقط عند التحقق من حالة الجهاز.
''';
...
# قيود إضافية
- لا تقترح أوامر v7-only (مثل WireGuard, BGP connection, routing table).
- لا تستخدم `placeholders` في أوامر الفحص — استخدم قيم حقيقية من snapshot.
- إذا تعارضت بيانات المستخدم مع snapshot، اعتمد snapshot كمصدر للحقيقة.
''';
